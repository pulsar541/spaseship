...
public class Level {
   ...
   // ArrayList<ArrayList<Integer>> listOfLists = new ArrayList<>(levelSizeHorCount());
    ...
    static public int STARS_COUNT = 1;
    ...
    public Vec3 generate(float excludeAreaRadius, int difficulty) {
        ...
        boolean someStarFullRandomPosFlag = true;
       ...
       // mustdel = new boolean[levelSizeHorCount()][levelSizeVertCount()];
        ...
        Vec3 tmpStarPos = new Vec3(i_cut * elementSize, j_cut * elementSize, 0);

        STARS_COUNT =  Math.max(3, levelSizeHorCount() / 20);


        int dir =  G_LEFT;
        int stepWeight = 1;
        int oldDir = 0;
        int maxSteps = levelSizeHorCount() * levelSizeVertCount() / 4;
...
*/
            ...
            if(isGameElement(i_cut, j_cut)) {
                table[i_cut][j_cut] = 0;
                tmpStarPos = new Vec3(i_cut * elementSize, j_cut * elementSize, 0);


                if(step > 2*maxSteps/3 && Math.random() > 0.95f && someStarFullRandomPosFlag) {
                    someStarFullRandomPosFlag = false;
                    if (!potencialStarPositions.contains(tmpStarPos)) {
                        potencialStarPositions.add(tmpStarPos);
                    }
                }
            ...
            for (int ly = 0; ly < levelSizeVertCount(); ly++) {
                Vec3 tmpPos = new Vec3(lx * elementSize, ly * elementSize, 0);
                 if(table[lx][ly] != 0) { // if(Math.random() > 0.9f && kepler.Math.farThen(tmpPos, excludeAreaCenterPos, excludeAreaRadius )) {
                    ...
                    if(difficulty >= 20) takesLifeCount =  Math.random() > 0.1f ? 0 : 100;
                    if(difficulty >= 40) takesLifeCount =  Math.random() > 0.33f ? 0 : 100;
                    if(difficulty >= 60) takesLifeCount =  Math.random() > 0.65f ? 0 : 100;
